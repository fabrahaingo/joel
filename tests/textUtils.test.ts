...
describe("Text Utils - Stopwords and Title Parsing", () => {
  describe("normalizeFrenchTextWithStopwords", () => {
    it("should remove common French stopwords", () => {
      const text = "Arrêté du 15 janvier 2024 relatif à la promotion dans le corps des ingénieurs";
  ...
  describe("parsePublicationTitle", () => {
    it("should extract publication type and clean title", () => {
      const title = "Arrêté du 6 janvier 2026 fixant le taux de promotion dans le corps";
    ...
    it("should handle titles with different date formats", () => {
      ...
      const { type: type1, cleanedTitle: cleanedTitle1 } = parsePublicationTitle(title1);
    ...
    it("should preserve text after date is removed", () => {
      const title = "Ordonnance du 1er mars 2024 relative à la simplification du droit";
      const { type, cleanedTitle } = parsePublicationTitle(title);
  ...
  describe("Integration: Stopwords + Title Parsing", () => {
    it("should work together for realistic publication titles", () => {
      const title = "Arrêté du 6 janvier 2026 fixant le taux de promotion dans le corps des ingénieurs de l'armement";
  ...
  describe("Performance comparison", () => {
    it("stopword removal should reduce word count significantly", () => {
      const title = "Arrêté du 6 janvier 2026 relatif à la promotion dans le corps des ingénieurs de l'armement";
      
      const normalizedStandard = normalizeFrenchText(title);
      const normalizedWithStopwords = normalizeFrenchTextWithStopwords(title);
      
      const standardWordCount = normalizedStandard.split(" ").filter(Boolean).length;
      const stopwordsWordCount = normalizedWithStopwords.split(" ").filter(Boolean).length;
      ...
      const reduction = (standardWordCount - stopwordsWordCount) / standardWordCount;
