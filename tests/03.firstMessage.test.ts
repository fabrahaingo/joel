...
const createMockSession = (hasUser: boolean): ISession => {
  const session = {
    ...
    loadUser: jest.fn().mockResolvedValue(hasUser ? { _id: "mockUserId" } : null),
    createUser: jest.fn(),
...
describe("First Message Processing", () => {
  ...
  describe("processMessage with isFirstMessage flag", () => {
    it("should treat invalid command as /start for first message", async () => {
      ...
      await processMessage(session, "random nonsense message", { isFirstMessage: true });
      ...
      const calls = (session.sendMessage as jest.MockedFunction<any>).mock.calls;
    ...
    it("should use default command for invalid message when NOT first message", async () => {
      ...
      await processMessage(session, "random nonsense message", { isFirstMessage: false });
