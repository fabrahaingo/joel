...
module.exports = bot => async msg => {
    ...
    try {
        ...
        if (!user || user.length === 0) {
            ...
            if (peoples.length === 0) {
                return bot.sendMessage(chatId, `Vous ne suivez aucun contact pour le moment. Tapez /start puis cliquez sur *🏃 Ajouter un contact* pour commencer à suivre des contacts.`, startKeyboard)
            } else {
        ...
        return await bot.onReplyToMessage(chatId, question_id, async msg => {
            ...
            if (isNaN(userAnswer)) {
                ...
                bot.sendMessage(chatId, "La réponse donnée n'est pas sous forme de nombre. Veuillez taper /start puis réessayer.", startKeyboard)
                return
            }
            // id to remove
            ...
            user.followedPeople = user.followedPeople.filter(p => !p.peopleId.equals(idToRemove))
            await user.save()
            bot.sendMessage(chatId, `Vous ne suivez plus *${nom} ${prenom}* 🙅‍♂️`, startKeyboard)
        })
    } catch (error) {
        console.log(error)
    }
