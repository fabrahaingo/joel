...
import { callJORFSearchDay, uniqueMinimalNameInfo } from "../utils/JORFSearch.utils";
...
function extractTaggedItems(JORF_items: JORFSearchItem[], tagName: FunctionTags, tagvalue?: string) {
  if (tagvalue === undefined) {
    return JORF_items.filter((item) =>
...
export function buildTagMap(updatedRecords: JORFSearchItem[], tagList: FunctionTags[]) {
...
async function updateUserFollowedPeople(user: IUser, updatedPeopleIds: Types.ObjectId[]) {
  ...
  user.followedPeople=user.followedPeople.reduce((followedList: {peopleId: ObjectId, lastUpdate: Date}[], followed)=>{
    if (followedList.some(f=>f.peopleId.toString()===followed.peopleId.toString())) return followedList; // If user follows twice the same person: we drop the second record

    // if updated people: we update the timestamp
    if (updatedPeopleIds.some(p=>p._id.toString()===followed.peopleId.toString())) {
      followedList.push({ peopleId: followed.peopleId, lastUpdate: currentDate});
    } else {
...
export async function notifyFunctionTagsUpdates(updatedRecords: JORFSearchItem[]
) {

  const updatedTagMap = buildTagMap(updatedRecords, Object.values(FunctionTags) as FunctionTags[]);
...
export async function notifyPeopleUpdates(updatedRecords: JORFSearchItem[]) {
  ...
  for (const user of updatedUsers) {
    ...
    const peopleIdsFollowedByUser=user.followedPeople.map(j=>j.peopleId.toString());
    const peopleFollowedByUser=updatedPeopleList.filter(i=>peopleIdsFollowedByUser.includes(i._id.toString()));
    const peopleInfoFollowedByUser=uniqueMinimalNameInfo(peopleFollowedByUser)

    // Records which are associated with followed People, and which are new for this respective People follow
    const newRecordsFollowedByUser=
        updatedRecords.reduce((recordList: JORFSearchItem[], record) => {
          // remove records not associated with followed people
          // this the first main filter
          if (!peopleInfoFollowedByUser.some(p=> p.nom === record.nom && p.prenom === record.prenom)) return recordList

          const updatesPeopleId=peopleFollowedByUser
              .find(i=>i.nom == record.nom && i.prenom == record.prenom)?._id;
          if (updatesPeopleId === undefined) return recordList; // this should not happen

          // Find the follow data associated with this People
          const followData=user.followedPeople.find(i=> i.peopleId.toString()===updatesPeopleId.toString());
    ...
    const updatedRecordsPeopleId=peopleFollowedByUser
        .filter(p=>newRecordsFollowedByUser.some(r=>( r.nom === p.nom && r.prenom === p.prenom)))
        .map(p=>p._id);
...
async function sendTagUpdates(user: IUser, tagMap: [[FunctionTags], JORFSearchItem[]][]) {

  // only keep the tags followed by the user
  const tagList = (Object.keys(tagMap) as FunctionTags[]).filter(tag=>user.followedFunctions.includes(tag));

  if (tagList.length == 0) {
    return;
  }

  let notification_text = "📢 Nouvelles publications parmi les fonctions que suivez :\n\n";
...
(async () => {
  ...
  JORFAllRecordsFromDate.sort((i,j)=>JORFtoDate(i.source_date).getTime()-JORFtoDate(j.source_date).getTime());
